...
`13d41e5` **HYPOTHESIS_VALUE tied to hiring reason** - The `HYPOTHESIS_VALUE` prompt was updated to explicitly instruct the LLM to frame candidate value against the challenge the hire exists to solve, not just the job description generically. Previous wording ("fill this gap") left the connection between value and hiring category implicit. The updated instruction ("directly addresses the challenge this hire exists to solve") makes the category load-bearing for both halves of the hypothesis. A Backfill hire should produce a value sentence about continuity and proven execution; a Recovery hire should produce one about turnaround record; a New capability hire should produce one about building without a playbook. README and build doc summary updated to match.

`CURRENT` **Company tracking data wired into hypothesis scoring** - The hypothesis category was previously inferred from posting language alone. The tracker database (`job_cache`) now feeds two signals into the scoring prompt before the LLM chooses a category: (1) repost count - how many times a similar title has been posted by this company in the dataset, and (2) active role count - how many distinct roles this company currently has open. A repost signal labels itself as suggesting Backfill or Recovery; multiple active roles labels itself as suggesting Capacity. The context block is only injected when there is a real signal - no history produces no block. `tracker.py` gained `get_company_posting_context()` (Jaccard title similarity >= 0.5, excludes current job by URL). `scorer.py` gained `_format_company_context()`, updated `score_job()` to accept `company_context`, and updated `score_all()` to query and pass context per job. The prompt placeholder `{company_context}` sits between the job description and the scoring framework so the LLM sees it before making a category decision. Signal improves as the tracker accumulates history - a fresh install has no data to contribute.
...
6. **Hypothesis** - YES and MAYBE jobs get a forced-choice hiring category (Backfill / Capacity / New capability / Recovery / Strategic bet / Unclear) with two labelled signals: why the company is hiring and what specific value the candidate brings to address the challenge that hire exists to solve. The category decision is informed by both posting language and tracker data - repost history and active role count are injected into the prompt as supporting signals when present.
